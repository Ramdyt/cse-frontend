...
export default function Documents() {
  ...
  return (
    <div onDragOver={e => { e.preventDefault(); setDragOver(true); }} onDragLeave={() => setDragOver(false)} onDrop={onDrop}>
      ...
      {/* Filtres thèmes (dynamiques) */}
      <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', marginBottom: 16, alignItems: 'center' }}>
        {themeFilters.map(t => {
          const themeObj = themes.find(th => th.name === t);
          return (
            <div key={t} style={{ display: 'flex', alignItems: 'center', gap: 2 }}>
              <button className={`theme-tab ${cat === t ? 'active' : ''}`} onClick={() => setCat(t)}>
                {themeObj && <span style={{ display: 'inline-block', width: 7, height: 7, borderRadius: '50%', background: themeObj.color, marginRight: 5 }} />}
                {t}
              </button>
              {t !== 'Tous' && (
                <button onClick={() => setShowDeleteTheme(themeObj)}
                  title="Supprimer ce thème"
                  style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'var(--muted)', fontSize: 13, padding: '2px 4px', borderRadius: 4 }}
        ...
        <button className="theme-tab" onClick={() => setShowNewTheme(true)}
          style={{ color: 'var(--accent)', borderColor: 'rgba(79,124,255,0.3)' }}>
          + Thème
        </button>
      </div>
