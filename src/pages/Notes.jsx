...
export default function Notes() {
  ...
  const [activeTab, setActiveTab] = useState('proposition');
          ...
          return (
            <button key={s.key} onClick={() => setActiveTab(s.key)}
              style={{ display:'flex', alignItems:'center', gap:6, padding:'7px 14px',
                ...
                background: isActive ? `${s.dot}22` : 'transparent',
                color: isActive ? s.dot : 'var(--muted)',
                fontWeight: isActive ? 600 : 400,
                borderBottom: isActive ? `2px solid ${s.dot}` : '2px solid transparent' }}>
              <div style={{ width:8, height:8, borderRadius:'50%', background:s.dot, flexShrink:0 }} />
              {s.label}
              <span style={{ background: isActive ? s.dot : 'var(--surface2)',
          ...
          <div key={s.key}>
            {colNotes.length === 0 && (
              <div className="empty-state">
                <div className="icon">{s.key === 'proposition' ? '💡' : s.key === 'discussion' ? '💬' : s.key === 'validee' ? '✅' : '❌'}</div>
                <div>Aucune note dans cette catégorie</div>
              </div>
            )}
              ...
              {colNotes.map(note => {
                ...
                return (
                  <div key={note.id} className="note-card" onClick={() => openEdit(note)}>
                    ...
                    {note.content && <div className="note-content">{note.content.slice(0, 100)}{note.content.length > 100 ? '…' : ''}</div>}
