...
import { getNotes, createNote, updateNote, deleteNote, getThemes, createTheme, deleteTheme, getMeetings } from '../api';
...
export default function Notes() {
  ...
  const [form, setForm] = useState({ title: '', content: '', status: 'proposition', theme: '', meeting_id: '' });
  const [saving, setSaving]                 = useState(false);

  const [showStatusModal, setShowStatusModal]   = useState(false);
  const [pendingStatus, setPendingStatus]       = useState({ note: null, newStatus: '' });
  const [selectedMeeting, setSelectedMeeting]   = useState('');

  const [showThemeFilter, setShowThemeFilter] = useState(false);

  // Colonnes réductibles
  const [collapsedCols, setCollapsedCols] = useState({ discussion: true, validee: true, refusee: true });
  ...
  useEffect(() => {
    Promise.all([getThemes(), getMeetings()]).then(([t, m]) => {
      setThemes(t); setMeetings(m);
      setForm(f => ({ ...f, theme: f.theme || t[0]?.name || '' }));
    });
    loadNotes();
  }, []);

  useEffect(() => { if (!loading) loadNotes(); }, [theme]);

  const loadNotes = () => {
    const params = theme !== 'Tous' ? { theme } : {};
    return getNotes(params).then(setNotes).finally(() => setLoading(false));
  };

  const openNew = () => {
    const defaultTheme = theme !== 'Tous' ? theme : (themes[0]?.name || '');
    setEditNote(null);
    setForm({ title: '', content: '', status: 'proposition', theme: defaultTheme, meeting_id: '' });
    setShowModal(true);
  };

  const openEdit = (note) => {
    setEditNote(note);
    setForm({ title: note.title, content: note.content, status: note.status, theme: note.theme, meeting_id: note.meeting_id || '' });
    setShowModal(true);
  };
  ...
  const applyStatus = async (note, newStatus, meetingId) => {
    const updated = await updateNote(note.id, { status: newStatus, meeting_id: meetingId || null });
  ...
  return (
    <div>
      <div className="notes-toolbar" style={{ display:'flex', alignItems:'center', gap:8, marginBottom:16 }}>
        {/* Bouton filtre thème */}
            ...
            {theme !== 'Tous' ? (
              <span style={{ display:'flex', alignItems:'center', gap:4 }}>
                <span style={{ width:8, height:8, borderRadius:'50%',
                  background: themes.find(t=>t.name===theme)?.color || 'var(--accent)', display:'inline-block' }} />
                {theme}
              </span>
            ) : 'Thème'}
          ...
          {showThemeFilter && (
            <>
              ...
              <div style={{ position:'absolute', top:'110%', left:0, background:'var(--surface)',
                ...
                {/* Tous */}
                <button onClick={() => { setTheme('Tous'); setShowThemeFilter(false); }}
                  style={{ width:'100%', display:'flex', alignItems:'center', gap:10, padding:'10px 14px',
                    background: theme==='Tous'?'var(--surface2)':'transparent', border:'none', cursor:'pointer',
                    color: theme==='Tous'?'var(--accent)':'var(--text)', fontSize:13,
                    fontFamily:"'DM Sans',sans-serif", fontWeight: theme==='Tous'?600:400, textAlign:'left' }}>
                  <span style={{ width:8, height:8, borderRadius:'50%', background:'var(--border)', display:'inline-block' }} />
                  Tous
                </button>
                {themes.map(t => (
                  <div key={t.id} style={{ display:'flex', alignItems:'center' }}>
                    <button onClick={() => { setTheme(t.name); setShowThemeFilter(false); }}
                      style={{ flex:1, display:'flex', alignItems:'center', gap:10, padding:'10px 14px',
                        background: theme===t.name?'var(--surface2)':'transparent', border:'none', cursor:'pointer',
                        color: theme===t.name?'var(--accent)':'var(--text)', fontSize:13,
                        fontFamily:"'DM Sans',sans-serif", fontWeight: theme===t.name?600:400, textAlign:'left' }}>
                    ...
                    <button onClick={() => { setShowDeleteTheme(t); setShowThemeFilter(false); }}
                      style={{ background:'none', border:'none', cursor:'pointer', color:'var(--muted)',
                        fontSize:13, padding:'10px 10px', lineHeight:1 }}
        ...
        <button className="btn btn-primary ml-auto" onClick={openNew}>+ Nouvelle note</button>
      </div>

      {/* Grille kanban avec colonnes réductibles */}
      <div style={{ display: 'grid', gridTemplateColumns: STATUSES.map(s => collapsedCols[s.key] ? '42px' : '1fr').join(' '), gap: 10, transition: 'grid-template-columns 0.3s ease' }}>
        {STATUSES.map(s => {
          ...
          return (
            <div key={s.key} style={{ minWidth: 0, overflow: 'hidden' }}>
              {/* En-tête de colonne */}
              <div className="notes-col-header" style={{ cursor: 'pointer', userSelect: 'none', justifyContent: 'space-between', padding: '6px 8px', borderRadius: 8, background: isCollapsed ? `${s.dot}22` : 'transparent', border: `1px solid ${isCollapsed ? s.dot+'44' : 'transparent'}`, transition: 'all 0.2s', marginBottom: 10, gap: 6 }}
                onClick={() => toggleCol(s.key)}>
                {isCollapsed ? (
                  /* Vue réduite — vertical */
                  <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 6, width: '100%' }}>
                    <div className="col-dot" style={{ background: s.dot }} />
                    <span style={{ fontSize: 10, color: s.dot, fontWeight: 700 }}>{colNotes.length}</span>
                    <span style={{ fontSize: 9, color: 'var(--muted)', writingMode: 'vertical-rl', transform: 'rotate(180deg)', letterSpacing: '0.05em', marginTop: 4 }}>
                  ...
                  <>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 6, flex: 1 }}>
                      <div className="col-dot" style={{ background: s.dot }} />
                      <span>{s.label}</span>
                      <span style={{ fontSize: 11, color: 'var(--muted)', marginLeft: 2 }}>{colNotes.length}</span>
                    </div>
              ...
              {!isCollapsed && (
                <div>
                  {colNotes.map(note => {
                    const themeColor = themes.find(t => t.name === note.theme)?.color || 'var(--accent)';
                    return (
                      <div key={note.id} className="note-card" onClick={() => openEdit(note)}>
                        <div className="note-title">{note.title}</div>
                        {note.content && <div className="note-content">{note.content.slice(0, 80)}{note.content.length > 80 ? '…' : ''}</div>}
                        {note.meeting_title && <div style={{ fontSize: 11, color: 'var(--accent)', marginBottom: 6 }}>📅 {note.meeting_title}</div>}
                        <div className="note-footer">
                          <span style={{ display: 'inline-flex', alignItems: 'center', gap: 4, padding: '2px 8px', borderRadius: 20, fontSize: 10, fontWeight: 500, background: `${themeColor}22`, color: themeColor }}>
                            <span style={{ width: 5, height: 5, borderRadius: '50%', background: themeColor, display: 'inline-block' }} />{note.theme}
                          </span>
                          <span className="note-author">{note.author?.name?.split(' ')[0]}</span>
                        </div>
                        {note.created_at && (
                          <div style={{ fontSize: 10, color: 'var(--border)', marginTop: 6 }}>
                            🕐 {new Date(note.created_at).toLocaleDateString('fr-FR', { day:'2-digit', month:'2-digit', year:'numeric' })}
                          </div>
                        )}
                        <div style={{ display: 'flex', gap: 4, marginTop: 10, flexWrap: 'wrap' }} onClick={e => e.stopPropagation()}>
                          {STATUSES.filter(st => st.key !== s.key).map(st => (
                            <button key={st.key} className="btn btn-ghost"
                              style={{ fontSize: 10, padding: '4px 8px', flex: '1 1 auto', textAlign: 'center', justifyContent: 'center' }}
                              onClick={() => requestStatusChange(note, st.key)}>
                              {st.label}
                            </button>
                          ))}
                          <button className="btn btn-danger"
                            style={{ fontSize: 10, padding: '4px 8px', width: '100%', marginTop: 2, textAlign: 'center', justifyContent: 'center' }}
                            onClick={() => remove(note)}>
                            🗑 Supprimer
                          </button>
                        </div>
                  ...
                  {colNotes.length === 0 && (
                    <div style={{ padding: 16, textAlign: 'center', color: 'var(--border)', fontSize: 12 }}>—</div>
                  )}
                </div>
              )}
            </div>
      ...
      {showModal && (
        <div className="modal-overlay">
          <div className="modal" onClick={e => e.stopPropagation()}>
            <div className="modal-title">{editNote ? 'Modifier la note' : 'Nouvelle note'}</div>
            <div className="form-group">
              <label className="form-label">Titre *</label>
              <input className="form-input" value={form.title} autoFocus onChange={e => setForm(f => ({ ...f, title: e.target.value }))} placeholder="Ex: Amélioration des horaires…" />
            </div>
            <div className="form-group">
              <label className="form-label">Description</label>
              <textarea className="form-textarea" value={form.content} onChange={e => setForm(f => ({ ...f, content: e.target.value }))} placeholder="Détails…" />
            </div>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
              <div className="form-group">
                <label className="form-label">Thème</label>
                <div style={{ display:'flex', gap:6 }}>
                  <select className="form-select" style={{ flex:1 }} value={form.theme} onChange={e => setForm(f => ({ ...f, theme: e.target.value }))}>
                    {themes.map(t => <option key={t.id} value={t.name}>{t.name}</option>)}
                  </select>
                  <button type="button" onClick={() => setShowNewTheme(true)}
                    style={{ background:'none', border:'1px solid var(--border)', borderRadius:8,
                      padding:'0 10px', cursor:'pointer', color:'var(--accent)', fontSize:18,
                      flexShrink:0, lineHeight:1 }}
                    title="Créer un nouveau thème">+</button>
                </div>
              </div>
              <div className="form-group">
            ...
            {form.status !== 'proposition' && (
              <div className="form-group">
                ...
                <select className="form-select" value={form.meeting_id} onChange={e => setForm(f => ({ ...f, meeting_id: e.target.value }))}>
                  <option value="">— Autre / Aucune réunion —</option>
                  {[...meetings].sort((a,b) => a.date.localeCompare(b.date)).map(m => {
                    const [y,mo,d] = m.date.split('-');
                    return <option key={m.id} value={m.id}>{d}/{mo}/{y} · {m.title}</option>;
                  })}
                </select>
              </div>
            )}
            <div className="modal-footer">
              ...
              <button className="btn btn-primary" onClick={save} disabled={saving}>{saving ? 'Enregistrement…' : editNote ? 'Modifier' : 'Ajouter'}</button>
            </div>
          </div>
        </div>
      )}
        ...
        <div className="modal-overlay">
          <div className="modal" onClick={e => e.stopPropagation()} style={{ maxWidth: 420 }}>
            ...
            <div style={{ fontSize: 13, color: 'var(--muted)', marginBottom: 18, lineHeight: 1.6 }}>
              La note <strong style={{ color: 'var(--text)' }}>"{pendingStatus.note?.title}"</strong> passe en <strong style={{ color: 'var(--text)' }}>{STATUSES.find(s => s.key === pendingStatus.newStatus)?.label}</strong>.<br />Souhaitez-vous la lier à une réunion ?
            </div>
            <div className="form-group">
              <label className="form-label">Réunion liée</label>
              <select className="form-select" value={selectedMeeting} onChange={e => setSelectedMeeting(e.target.value)}>
                <option value="">— Autre / Aucune réunion —</option>
                {[...meetings].sort((a,b) => a.date.localeCompare(b.date)).map(m => {
                  const [y,mo,d] = m.date.split('-');
                  return <option key={m.id} value={m.id}>{d}/{mo}/{y} · {m.title}</option>;
            ...
            <div className="modal-footer">
              <button className="btn btn-ghost" onClick={() => setShowStatusModal(false)}>Annuler</button>
              <button className="btn btn-primary" onClick={() => applyStatus(pendingStatus.note, pendingStatus.newStatus, selectedMeeting)}>Confirmer</button>
            </div>
          </div>
        </div>
      )}
            ...
            <div className="form-group">
              ...
              {themeForm.name && (
                <div style={{ marginTop: 10, display: 'inline-flex', alignItems: 'center', gap: 6, padding: '4px 12px', borderRadius: 20, background: `${themeForm.color}22`, color: themeForm.color, fontSize: 12, fontWeight: 500 }}>
                  <span style={{ width: 7, height: 7, borderRadius: '50%', background: themeForm.color, display: 'inline-block' }} />{themeForm.name}
                </div>
              )}
            </div>
            <div className="modal-footer">
              <button className="btn btn-ghost" onClick={() => setShowNewTheme(false)}>Annuler</button>
              <button className="btn btn-primary" onClick={saveTheme} disabled={saving || !themeForm.name.trim()}>{saving ? '…' : 'Créer'}</button>
            </div>
          </div>
        </div>
      )}
        ...
        <div className="modal-overlay" onClick={() => setShowDeleteNote(null)}>
          <div className="modal" onClick={e => e.stopPropagation()} style={{ maxWidth: 420 }}>
            ...
            <div style={{ fontSize: 14, color: 'var(--muted)', marginBottom: 20, lineHeight: 1.6 }}>
              Voulez-vous supprimer la note <strong style={{ color: 'var(--text)' }}>"{showDeleteNote.title}"</strong> ?<br />
              <span style={{ fontSize: 13, color: 'var(--red)' }}>⚠️ Cette action est irréversible.</span>
            </div>
            <div className="modal-footer">
              <button className="btn btn-ghost" onClick={() => setShowDeleteNote(null)}>Annuler</button>
