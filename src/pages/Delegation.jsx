...
export default function Delegation() {
  ...
  const load = async () => {
    ...
    try {
      ...
      setSummary(
        s || {
          members: [],
          pool: { total: 0, taken: 0, remaining: 0, count: 0, hours_each: 7 },
        },
      );
      setEntries(e || []);
      setTransfers(t || []);
      ...
      setLoadError(
        "Impossible de charger les données de délégation. Vérifiez que le serveur est démarré.",
      );
    } finally {
      setLoading(false);
    }
  ...
  const saveEntry = async () => {
    ...
    if (isMutualisation && !me?.titulaire) {
      ...
      try {
        ...
        notify(
          "✅ Demande de mutualisation envoyée — le titulaire doit la valider",
        );
        load();
      } catch (e) {
    ...
    try {
      ...
      setEntryForm({
        taker_id: String(me?.id || ""),
        source: me?.titulaire ? "own" : "pool",
        owner_id: me?.titulaire ? String(me?.id || "") : "",
        hours: "",
        date: now_str,
        description: "",
      });
  ...
  const saveTransfer = async () => {
    if (!transferForm.from_id || !transferForm.to_id || !transferForm.hours)
      return;
    setSaving(true);
    try {
      await createTransfer({
  ...
  const handleApprove = async (id, status) => {
    try {
      ...
      notify(
        status === "approved"
          ? "✅ Mutualisation approuvée — email envoyé à la RH"
          : "❌ Mutualisation refusée",
      );
      load();
    } catch (e) {
      notify("❌ " + (e.response?.data?.error || "Erreur"));
  ...
  const pendingForMe = transfers.filter(
    (t) => t.status === "pending" && t.from_id === me?.id,
  );

  const getMemberBalance = (userId) =>
    summary?.members?.find((m) => m.user.id === userId)?.balance;
    ...
    <div>
      ...
      >
        ...
        <div style={{ display: "flex", gap: 6, flex: 1, flexWrap: "wrap" }}>
          {[
            ...
            <button
              key={id}
              className={`theme-tab ${tab === id ? "active" : ""}`}
      ...
      {pendingForMe.length > 0 && (
        ...
        >
          ...
          >
            ⏳ {pendingForMe.length} demande(s) de mutualisation en attente de
            votre validation
          </div>
          {pendingForMe.map((t) => (
            <div
            ...
            >
              <div className="avatar sm">
                {t.to_avatar || t.to_name?.slice(0, 2)}
              </div>
              <span style={{ flex: 1, fontSize: 13 }}>
                <strong>{t.to_name}</strong> demande{" "}
                <strong style={{ color: "var(--orange)" }}>{t.hours}h</strong>
                {t.note ? ` — ${t.note}` : ""}
              </span>
              <button
                className="btn btn-primary"
        ...
        <div>
          ...
          {summary?.pool &&
            (() => {
              ...
              const suppleants = summary.members.filter(
                (m) => !m.user.titulaire,
              );
                ...
                <div className="card" style={{ marginBottom: 20 }}>
                  ...
                  >
                    ...
                    <div style={{ fontSize: 13, color: "var(--muted)" }}>
                      Pot fixe : {poolTotal}h/mois · {suppleants.length}{" "}
                      suppléant(s)
                    </div>
                  </div>
                  {poolTotal === 0 ? (
                    <div style={{ fontSize: 13, color: "var(--muted)" }}>
                      Aucun suppléant enregistré.
                    </div>
                  ) : (
                    <div
                      style={{
          ...
          summary.members.filter((m) => m.user.titulaire).length === 0 ? (
            <div className="empty-state">
              ...
              <div>
                ...
                <br />
                Allez dans Administration pour définir les membres comme
                Titulaires.
              </div>
            </div>
          ) : (
            ...
            >
              {summary.members
                .filter((m) => m.user.titulaire)
                .map(({ user, balance: rawBal }) => {
                    ...
                    >
                      ...
                      >
                        {[
                          ...
                          bal.reported > 0
                            ? ["Report", `+${bal.reported}h`, "var(--accent)"]
                            : null,
                          ["Pris", `${bal.taken}h`, "var(--orange)"],
                          bal.transferred_out > 0
                            ? [
                                "Donnés",
                                `-${bal.transferred_out}h`,
                                "var(--red)",
                              ]
                            : null,
                          bal.transferred_in > 0
                            ? [
                                "Reçus",
                                `+${bal.transferred_in}h`,
                                "var(--green)",
                              ]
                            : null,
                        ]
      ...
      {tab === "history" && (
        <div className="card">
          ...
          >
            <div className="section-title" style={{ marginBottom: 0 }}>
              Historique — {MONTHS[month - 1]} {year}
            </div>
            <div style={{ fontSize: 13, color: "var(--muted)" }}>
          ...
          {entries.length === 0 ? (
            <div className="empty-state">
              ...
              <div>Aucune prise d'heures ce mois</div>
            </div>
          ) : (
            <div style={{ overflowX: "auto" }}>
              <div
                style={{
                  display: "grid",
                  gridTemplateColumns: "90px 1fr 1fr 70px 1fr 50px",
                  minWidth: 500,
                  padding: "8px 14px",
                  fontSize: 11,
                  color: "var(--muted)",
                ...
                >
                  ...
                  >
                    <div className="avatar xs">
                      {e.taker_avatar || e.taker_name?.slice(0, 2)}
                    </div>
      ...
      {tab === "transfers" && (
        <div className="card">
          <div className="section-title">Historique des mutualisations</div>
          {transfers.length === 0 ? (
            <div className="empty-state">
              <div className="icon">🔄</div>
              <div>Aucune mutualisation</div>
            </div>
          ) : (
            transfers.map((t) => {
              const sc = {
              ...
              return (
                ...
                >
                  ...
                  >
                    ...
                    <div className="avatar xs">
                      {t.from_avatar || t.from_name?.slice(0, 2)}
                    </div>
                    <span style={{ fontSize: 13, fontWeight: 500 }}>
                      {t.from_name}
                    </span>
        ...
        <div>
          ...
          <div className="card" style={{ marginBottom: 16 }}>
            <div className="section-title">
              ⚖️ Ajuster les compteurs titulaires
            </div>
            <div
              style={{
                display: "grid",
          ...
          <div className="card" style={{ marginBottom: 16 }}>
            ...
            >
              <div className="section-title" style={{ marginBottom: 0 }}>
                🛠 Toutes les saisies (correction admin)
              </div>
              <button
                className="btn btn-primary"
                style={{ fontSize: 12 }}
            ...
            {allEntries.length === 0 ? (
              ...
              <div style={{ overflowX: "auto" }}>
                <div
                  style={{
                    display: "grid",
                    gridTemplateColumns: "90px 1fr 1fr 70px 1fr 80px",
                    minWidth: 550,
                    padding: "8px 14px",
                    fontSize: 11,
                    color: "var(--muted)",
                ...
                {allEntries.map((e) => (
                  ...
                  >
                    ...
                    <span style={{ fontSize: 13 }}>
                      {e.is_pool ? "🫙 Pot commun" : e.owner_name || "—"}
                    </span>
                    <span
                      style={{
                        fontSize: 15,
      ...
      {showEntry && (
        <div className="modal-overlay">
          <div className="modal" onClick={(e) => e.stopPropagation()}>
                  ...
                  {me?.titulaire ? (
                    <>
                      ...
                      <strong style={{ color: "var(--green)" }}>
                        {getMemberBalance(me?.id)?.remaining ?? 20}h disponibles
                      </strong>
                    </>
                  ) : (
                    <>
                      Suppléant · Pot commun :{" "}
                      <strong style={{ color: "var(--accent2)" }}>
                        {summary?.pool?.remaining ?? 7}h disponibles
                      </strong>
                    </>
                  )}
                </div>
              ...
              <div className="form-group">
                ...
                >
                  <button
                    ...
                    style={{
                      padding: "10px 14px",
                      border: `1px solid ${entryForm.source === "own" ? "var(--accent)" : "var(--border)"}`,
                      borderRadius: 8,
                      background:
                        entryForm.source === "own"
                  ...
                  >
                    ...
                    >
                      {getMemberBalance(me?.id)?.remaining ?? 20}h dispo
                    </span>
                  </button>
                  <button
                    type="button"
                    ...
                    style={{
                      padding: "10px 14px",
                      border: `1px solid ${entryForm.source === "other" ? "var(--accent)" : "var(--border)"}`,
                      borderRadius: 8,
                      background:
                        entryForm.source === "other"
                          ? "rgba(79,124,255,0.12)"
              ...
              <div className="form-group">
                ...
                >
                  <button
                    ...
                    style={{
                      padding: "10px 14px",
                      border: `1px solid ${entryForm.source === "pool" ? "var(--accent2)" : "var(--border)"}`,
                      borderRadius: 8,
                      background:
                        entryForm.source === "pool"
                  ...
                  >
                    ...
                    >
                      {summary?.pool?.remaining ?? 7}h dispo
                    </span>
                  </button>
                  <button
                    type="button"
                  ...
                  >
                    🔄 Mutualisation depuis un titulaire
                    <div
                      style={{
                        fontSize: 11,
                        color: "var(--muted)",
                    ...
                    >
                      Le titulaire devra valider la demande
                    </div>
                  </button>
                </div>
              </div>
            ...
            {entryForm.source === "other" && (
              <div className="form-group">
                <label className="form-label">
                  {me?.titulaire ? "Titulaire à débiter" : "Titulaire donneur"}{" "}
                  *
                </label>
                <select
                  className="form-select"
                  value={entryForm.owner_id}
                  onChange={(e) =>
                    setEntryForm((f) => ({ ...f, owner_id: e.target.value }))
                  }
                >
                    ...
                    .map((u) => {
                      ...
                      return (
                        <option key={u.id} value={u.id}>
                          {u.name} ({bal ? `${bal.remaining}h dispo` : "—"})
                        </option>
                      );
                    })}
                </select>
            ...
            {entryForm.source === "other" && !me?.titulaire && (
              <div
                style={{
                  padding: "10px 14px",
              ...
              >
                ⚠️ Une demande de mutualisation sera envoyée au titulaire. Les
                heures ne seront décomptées qu'après sa validation.
              </div>
            )}
            ...
            >
              📧 Un email sera automatiquement envoyé à la RH.
            </div>
            ...
            <div className="modal-footer">
              ...
              <button
                ...
                disabled={
                  saving ||
                  !entryForm.hours ||
                  (entryForm.source === "other" && !entryForm.owner_id)
                }
              >
                {saving
                  ? "Enregistrement…"
      ...
      {showTransfer && (
        <div className="modal-overlay">
          <div
            ...
            style={{ maxWidth: 460 }}
          >
            <div className="modal-title">Demande de mutualisation</div>
            <div
              style={{
                fontSize: 13,
                color: "var(--muted)",
            ...
            >
              Un membre peut demander à un titulaire de lui transférer des
              heures. Le titulaire devra valider.
            </div>
            <div
              style={{
                display: "grid",
            ...
            >
              <div className="form-group">
                ...
                <select
                  ...
                  onChange={(e) =>
                    setTransferForm((f) => ({ ...f, from_id: e.target.value }))
                  }
                >
                  <option value="">— Choisir —</option>
                  {titulaires.map((u) => {
              ...
              <div className="form-group">
                ...
                <select
                  ...
                  onChange={(e) =>
                    setTransferForm((f) => ({ ...f, to_id: e.target.value }))
                  }
                >
                  <option value="">— Choisir —</option>
                  {users.map((u) => (
                    <option key={u.id} value={u.id}>
                      {u.name} ({u.titulaire ? "Titulaire" : "Suppléant"})
                    </option>
                  ))}
                </select>
              </div>
            ...
            >
              <div className="form-group">
                ...
                <input
                  ...
                  onChange={(e) =>
                    setTransferForm((f) => ({ ...f, hours: e.target.value }))
                  }
                  placeholder="Ex: 3"
                />
              </div>
      ...
      {showConfig && me?.is_admin && (
        <div className="modal-overlay">
          <div className="modal" onClick={(e) => e.stopPropagation()}>
            ...
            >
              {[
                ["Heures/mois titulaires", "hours_titulaire"],
                ["Heures/mois pot suppléants", "hours_suppleant"],
                ["Plafond report (max)", "max_report"],
              ].map(([label, key]) => (
            ...
            >
              ...
              >
                Définit le premier mois où les compteurs démarrent à 20h sans
                report. Avant cette date = ignoré.
              </div>
              <div
                style={{
                  display: "grid",
            ...
            <div className="form-group">
              ...
              <input
                className="form-input"
                type="email"
                value={configForm.rh_email}
                onChange={(e) =>
                  setConfigForm((f) => ({ ...f, rh_email: e.target.value }))
                }
            ...
            >
              <div className="form-group">
                ...
                <input
                  ...
                  onChange={(e) =>
                    setConfigForm((f) => ({ ...f, smtp_host: e.target.value }))
                  }
            ...
            >
              <div className="form-group">
                ...
                <input
                  ...
                  onChange={(e) =>
                    setConfigForm((f) => ({ ...f, smtp_user: e.target.value }))
                  }
                  placeholder="cse@entreprise.fr"
                />
              </div>
              <div className="form-group">
                ...
                <input
                  ...
                  onChange={(e) =>
                    setConfigForm((f) => ({ ...f, smtp_pass: e.target.value }))
                  }
      ...
      {showAdminEdit && (
        <div className="modal-overlay">
          ...
          >
            ...
            {showAdminEdit === "new" && (
              <>
                <div className="form-group">
                  ...
                  <select
                    className="form-select"
                    value={adminForm.taker_id || ""}
                    onChange={(e) =>
                      setAdminForm((f) => ({ ...f, taker_id: e.target.value }))
                    }
                  >
                    <option value="">— Choisir —</option>
                    {users.map((u) => (
                ...
                <div className="form-group">
                  ...
                  <select
                    className="form-select"
                    value={adminForm.owner_id || ""}
                    onChange={(e) =>
                      setAdminForm((f) => ({
                        ...f,
                        owner_id: e.target.value,
                        is_pool: e.target.value === "pool",
                      }))
                    }
                  >
            ...
            >
              <div className="form-group">
                ...
                <input
                  ...
                  onChange={(e) =>
                    setAdminForm((f) => ({ ...f, hours: e.target.value }))
                  }
                  placeholder="Ex: 2.5"
                />
              </div>
              <div className="form-group">
                ...
                <input
                  ...
                  onChange={(e) =>
                    setAdminForm((f) => ({ ...f, date: e.target.value }))
                  }
                />
              </div>
            </div>
            ...
            >
              ⚠️ Mode admin — aucune vérification de solde. Les modifications
              sont tracées.
            </div>
                  ...
                  try {
                    if (showAdminEdit === "new") {
                      await adminCreateEntry({
                        ...
                        owner_id: adminForm.is_pool
                          ? null
                          : adminForm.owner_id
                            ? parseInt(adminForm.owner_id)
                            : parseInt(adminForm.taker_id),
                        is_pool:
                          adminForm.is_pool || adminForm.owner_id === "pool",
                        hours: parseFloat(adminForm.hours),
                        date: adminForm.date,
                        description:
                          adminForm.description || "[Correction admin]",
                      });
        ...
        <div className="modal-overlay">
          ...
          >
            ...
            >
              <div className="avatar sm">
                {showAdjust.avatar ||
                  showAdjust.name?.slice(0, 2).toUpperCase()}
              </div>
              <div>
                <div style={{ fontSize: 14, fontWeight: 600 }}>
                  {showAdjust.name}
                ...
                <div style={{ fontSize: 12, color: "var(--muted)" }}>
                  ...
                  / {getMemberBalance(showAdjust.id)?.total ?? 20}h
                </div>
              </div>
            </div>
            ...
            <div className="form-group">
              <label className="form-label">
                Nouveau solde souhaité (en heures) *
              </label>
              <input
                className="form-input"
                type="number"
              ...
              >
                {adjustTarget && getMemberBalance(showAdjust.id) ? (
                  <span
                    style={{
                      color:
                        parseFloat(adjustTarget) >
                        (getMemberBalance(showAdjust.id)?.remaining ?? 0)
                          ? "var(--green)"
                          : "var(--orange)",
                    }}
                  >
            ...
            >
              ⚠️ L'ajustement sera tracé dans l'historique comme une correction
              admin.
            </div>
                ...
                onClick={async () => {
                  ...
                  try {
                    const result = await adjustBalance({
                      ...
                      month,
                      note:
                        adjustNote || `[Ajustement admin → ${adjustTarget}h]`,
                    });
                    setShowAdjust(null);
                    notify(
                      `✅ Compteur de ${showAdjust.name} ajusté à ${result.new_remaining}h`,
                    );
                    load();
                  } catch (e) {
                    notify("❌ " + (e.response?.data?.error || "Erreur"));
